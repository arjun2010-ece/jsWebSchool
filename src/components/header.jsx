...
const NextArrow = (props) => {
  ...
  return (
    <div
      className={`${className} block bg-opacity-50 bg-black text-white`}
      style={style}
      onClick={onClick}
    >
      →
    ...
    <header>
      <nav className="bg-gray-200 border-gray-200 dark:bg-gray-900 dark:border-gray-700 sticky top-0 ">
        ...
        <div className="mx-auto flex max-w-screen-xl flex-wrap items-center justify-between p-4">
          ...
          {/* commentiong out below code for changes later */}
        ...
        {/* {subMenuVisible && (
          <HorizontalSubMenu subMenuVisible={subMenuVisible} />
        )} */}
          ...
          <div className="w-[90%]">
            <Slider {...settings}>
              {TUTORIALS.map((tutorial, index) => (
                <Link
                  key={index}
                  className="p-2 px-5 bg-black text-white rounded-lg text-center"
                  href={tutorial?.path}
                >
                  {tutorial?.title}
                </Link>
