...
type REACTJS_TOPICS_PROPS = {
  topic: string;
  path: string;
};

const REACTJS_TOPICS: REACTJS_TOPICS_PROPS[] = [
  { topic: 'What is ReactJs ?', path: '/reactjs' },
  { topic: 'Setting up react project', path: '/reactjs/setting-up-react-project' },
  { topic: 'Understanding JSX', path: '/reactjs/understanding-jsx' },
 
];

const ReactjsSidebar: FC = () => {
  const router: NextRouter = useRouter();

  const reactjsContent = REACTJS_TOPICS.map((course, i) => {
  ...
  return (
    <aside>
      <StyledListContainer>{reactjsContent}</StyledListContainer>
    </aside>
  );
};
